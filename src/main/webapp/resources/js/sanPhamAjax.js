$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/bookshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/bookshop/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					...
					var checkTenDanhMuc = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("Book".toLowerCase());
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btnCapNhatBook" >Cập nhật</button>':'<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
	...
	$('#danhMucDropdown').mouseup(function() {
		...
		if (open) {
			...
			if ((label.toLowerCase()).indexOf("Book".toLowerCase()) != -1) {
				$('.bookModal').modal('show');
				$("#idDanhBook").val($(this).val());
				$('#bookForm').removeClass().addClass("addBookForm");
	...
	$('.bookModal, .otherModal').on('hidden.bs.modal', function(e) {
		e.preventDefault();
		$("#idDanhMucBook, #idDanhMucKhac").val("");
		$("#idSanPhamBook, #idSanPhamKhac").val("");
			
	    $('#bookForm').removeClass().addClass("bookForm");
		$('#bookForm #btnSubmit').removeClass().addClass("btn btn-primary");
    ...
    function ajaxPostBook() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/bookshop/api/san-pham/save",
    ...
    // vs danh mục book
    $(document).on("click",".btnCapNhatBook", function(event){
		event.preventDefault();
		...
		var href = "http://localhost:8080/bookshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			populate('.updateBookForm', sanPham);
			$("#idDanhMucBook").val(sanPham.danhMuc.id);
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/bookshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/bookshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/bookshop/img/"+sanPham.id+".png");
